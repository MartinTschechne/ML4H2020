...
def get_CNN_model(seq_length, kernel_size=5):
    '''model_id: cnn-model'''
    model = Sequential()
    model.add(Conv1D(input_shape=(seq_length,1),filters=32, kernel_size=kernel_size, padding='same', activation='relu'))
...
def get_large_CNN_model(seq_length, kernel_size=5):
    '''model_id: cnn-model'''
    model = Sequential()
    model.add(Conv1D(input_shape=(seq_length,1),filters=32, kernel_size=kernel_size, padding='same', activation='relu'))
    model.add(Conv1D(filters=32, kernel_size=kernel_size, padding='same', activation='relu'))
...
def get_model(model_id, seq_length, kernel_size):
    if model_id == 'cnn-model':
        return get_CNN_model(seq_length, kernel_size)
    if model_id == 'cnn-model-large':
        return get_large_CNN_model(seq_length, kernel_size)
    elif model_id == 'debug':
